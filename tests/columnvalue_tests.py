...
def test_should_return_date_column_value_with_time():
    ...
    # Act
    column_value = cv.create_column_value(column_type, id=id, title=title, date=date, time=time)
    format = column_value.format()
...
def test_should_return_status_column_value_by_index():
    ...
    column_value = cv.create_column_value(
        ...
        title=title, 
        index=index, 
        settings=o.StatusSettings(**{'labels': {'2': 'Status Two'}}))
    format = column_value.format()
...
def test_should_return_status_column_value_by_label():
    ...
    column_value = cv.create_column_value(
        ...
        text=text, 
        index=3, 
        settings=o.StatusSettings(**{'labels': {'3': 'Status 3'}}))
    format = column_value.format()
...
def test_should_return_status_column_value_by_label_with_preference():
    ...
    column_value = cv.create_column_value(
        ...
        index=4, 
        text=text, 
        settings=o.StatusSettings(**{'labels': {'4': "Status 4"}}))
    format = column_value.format()
